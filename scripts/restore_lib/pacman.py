...
def export(ctx):
    ...
    # Reference only: hardware-specific repositories and IgnorePkg are not applied.
    atomic_write(root / "pacman.conf.reference", Path("/etc/pacman.conf").read_bytes())
    ...
    atomic_write(root / "export.sh", '#!/bin/bash\nset -euo pipefail\n'
                 'repo_dir=$(dirname -- "$(dirname -- "$(dirname -- "$(readlink -f -- "${BASH_SOURCE[0]}")")")")\n'
                 'exec "$repo_dir/install.sh" --export pacman "$@"\n', 0o755)
...
def preflight(ctx):
    ...
    missing = sorted(set(native) - available)
    if missing:
        raise RuntimeError("当前软件源找不到以下仓库包；先准备适合目标 CPU 的软件源并刷新数据库：\n  "
                           + " ".join(missing))
    if not (ctx.repo / "tools/.config/paru/paru.conf").is_file():
        raise RuntimeError("缺少个人 PKGBUILD 来源配置")
...
def restore_foreign(ctx):
    ...
    env = dict(os.environ, PARU_CONF=str(ctx.repo / "tools/.config/paru/paru.conf"))
    for name, reason in (("pkglist_aur.txt", "--asexplicit"), ("pkglist_aur_deps.txt", "--asdeps")):
        if lists[name]:
            ctx.run("paru", "--sudo", "run0", "-S", "--needed", reason, *lists[name], env=env)
