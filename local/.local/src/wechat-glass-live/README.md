...
```bash
python3 local/.local/src/wechat-glass-live/control.py status
python3 local/.local/src/wechat-glass-live/control.py enable
python3 local/.local/src/wechat-glass-live/control.py disable
python3 local/.local/src/wechat-glass-live/control.py opacity 0.52
```
...
需要保存日常调整时同步修改 `kde/kwinrc`，下一次复现会以该文件为准。
