...
Run the full install when upgrading to the native search implementation: it
adds `org.kde.plasma.appgrid.core` alongside the existing background-effect
module. Restart the launcher host after updating native modules. Once both
modules are installed, QML-only iterations can use a local KPackage upgrade:
